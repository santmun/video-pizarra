...
export default {
  id: 'acuarela-viva', name: 'Acuarela viva',
  ...
  palette: { bg: PAPER, ink: INK, accent: '#D83A34', muted: '#6E5E50', panel: '#FBF5EA', line: 'rgba(36,29,24,.2)', good: '#4E7D5B', bad: '#A4553A', mascot: '#D97757' },
  ...
  async setup(K) {
    ...
    const vars = [['#F6E7D9', '#D83A34', '#D97757'], ['#FFF7EF', '#1E1E1E', '#D97757'], ['#F6E7D9', '#1E1E1E', '#D83A34']];
    vars.forEach((cols, v) => {
      const c = L.canvas(W, H), x = c.getContext('2d'); x.drawImage(paperC, 0, 0);
      L.watercolor(x, L.rectPoly(-80, -60, W + 160, H * 0.75), cols[0], { seed: 2 + v, layers: 28, alpha: 0.06, amp: 0.2 });
      L.watercolor(x, L.circlePoly(W * (0.72 - v * 0.22), H * 0.26, W * 0.24, H * 0.24), cols[2], { seed: 5 + v, layers: 24, alpha: 0.07 });
      L.watercolor(x, L.rectPoly(-80, H * 0.76, W + 160, H * 0.35), cols[1], { seed: 7 + v, layers: 26, alpha: 0.07, amp: 0.15 });
      washes[v] = c;
    });
  ...
  mascot(K, box, p, s, mood) { // pops off any background: white sticker halo + solid Claude orange + thick ink
    if (p <= 0) return; const ctx = K.ctx, bt = boil(K), b2 = { ...box, y: box.y + (1 - L.E.back(p)) * 30 * K.u }, bob = Math.sin(K.t * 3) * 3 * K.u;
    ctx.save(); ctx.globalAlpha = L.clamp(p * 2);
    L.mascot(ctx, K.spec.mascot, b2, { fill: () => {}, stroke: (pth) => { ctx.save(); ctx.strokeStyle = '#FFFFFF'; ctx.lineWidth = 16 / Math.max(0.3, box.h / 180); ctx.stroke(pth); ctx.restore(); }, color: '#D97757', eye: 'rgba(0,0,0,0)', bob });
    const wf = L.washFill(ctx, 8, { alpha: 0.9, gran: 0.08, passes: 2 });
    L.mascot(ctx, K.spec.mascot, b2, { fill: (pth, c) => { ctx.fillStyle = '#D97757'; ctx.fill(pth); wf(pth, '#C8643F'); }, stroke: L.wobbleStroke(ctx, INK, 6, 1, bt), color: '#D97757', ink: INK, eyeStyle: mood === 'happy' ? 'happy' : 'dot', bob });
    ctx.restore();
  },
